...
algae_df %>% 
  ...
  mutate(species = str_sub(sample_id, 1, 1), 
         ...
         reef = str_sub(sample_id, 6,7)) -> algae_df
```


```{r}
GGally::ggpairs(select(algae_df, -3, -4), mapping = aes(color = Clade)) -> ggpairs_color_clade
```
...
```{r}
GGally::ggpairs(select(algae_df, -3, -4), mapping = aes(color = species)) -> ggpairs_color_species
```
