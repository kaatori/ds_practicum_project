...
### Start with balanced dataset first because will need to retransform after first 1000. 
...
lr_clr_df_1000 %>% 
  mutate(Clade = as.character(Clade)) -> lr_clr_df_1000

xfactors <- model.matrix(Clade ~ ., data = lr_clr_df_1000[, -1])
#x        <- as.matrix(data.frame(age, bmi_p, xfactors))
y <- as.factor(lr_clr_df_1000$Clade)

dim(xfactors) # 278 x 1001 bc of no sample id 

glm_lasso_1000 <- glmnet(x = xfactors, 
                         y = Clade, 
                         alpha = 1, # lasso
                         family = "binomial")
```
...
# Replace 'Response' with the actual name of your binary outcome variable
...
# Fit the LASSO model
lasso_model <- glmnet(x_matrix, y_vector, alpha = 1, family = "binomial")
