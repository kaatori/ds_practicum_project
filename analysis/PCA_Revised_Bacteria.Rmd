...
- **Note:** This section uses the original abundance counts per ASV. 
- The revised analysis uses proportions per ASV by sample row instead. Please refer to the file 'PCA_Revised_Bacteria_Proportions.Rmd' for that analysis instead of this one. 
...
```{r}
pca_out %>% glimpse() # list of 5

pca_out$center %>% as.data.frame()

pca_out$x %>% as.data.frame() # 659 x 659
...
glimpse(pca_out_summary) # list of 6
```
- From the summary of the PCA results, the output is hard to read because there are 659 samples. 

- Need to review the 'importance' part of the summary results, but it is not in an easily-readable format; therefore need to adjust.
...
```
- About 239 principal components explain 80 % of cumulative variance in the X matrix of only the 5K bacteria, but the increments here are so small per bacteria by glancing at the individual proportion of variance. 

- The number of principal components may be used as the number of ```k``` clusters to look for in K-Means clustering that will be done in separate .Rmd file. 
...
factoextra::fviz_pca_biplot(pca_out, repel = T, 
                          ...
                          #  xlim = c(-100, 300), 
                          #  ylim = c(),
                          title = "PCA Biplot\n Top 5 Contributors indicated by blue arrows",
...
importance_df %>% 
  ...
  geom_point() +
  ggtitle("PCA Results: Proportion of Variance", "This is a better screeplot showing all PCs on counts-based `rev_bacteria` Scaled") +
...
fviz_pca_var(pca_out, 
             ...
             alpha.var = .5,
             #repel = TRUE,
             title = "Count-based, Scaled Bacteria PCA Results \nTop 100 Contributors",
