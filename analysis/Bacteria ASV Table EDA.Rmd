...
map(bacteria[,-1], ~sum(.==0)) %>% as_tibble() %>% 
  ...
  # if the cut off includes 656 or more
  filter(sum_col_eq_0 >=656) -> ASV_cols_sum_eq_0

ASV_cols_sum_eq_0

###### if the cut of includes 657 or more because Dr. C said at least 3 samples
map(bacteria[,-1], ~sum(.==0)) %>% as_tibble() %>% 
  pivot_longer(cols = everything(), 
               names_to = "ASV_column", 
               values_to = "sum_col_eq_0") %>% 
  filter(sum_col_eq_0 >= 657) -> ASV_cols_sum_eq_0_657# because we want 656 to say: 656 zeros means that ASV was found in 3 samples. 
...
- The above list of 27,283 ASVs will be presented to Dr. Cárdenas for filtering out. 
