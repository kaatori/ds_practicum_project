...
- Next, need ot do this for the other columns
...
```{r}
sum(bacteria$ASV0212==0) # yes, 656 

sum(bacteria$ASV0864==0) # yes 657

sum(bacteria$ASV11284==0) # yes, 658

```

- The above list of 28,733 ASVs will be presented to Dr. Cárdenas for filtering out. 
- Save list of the ASV column names for later: 
```{r}
ASV_cols_sum_eq_0$ASV_column -> list_ASV_656

length(list_ASV_656) # 28,733 
```
...
bacteria %>% 
  ...
  # recommended method is to use all_of() but put (-) sign before it to NOT select those 28k ASVs
  select(-all_of(list_ASV_656))
```
