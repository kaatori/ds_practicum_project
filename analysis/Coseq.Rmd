...
- Takes a matrix of data.frame of gene-level or ASV-level counts of dimension n x d where n= genes or ASVs and d=samples: 
```{r}
...
# number of iter and number of lcusters is low to try out method
coseq(t_rev_bacteria_counts,
      K=2:25, 
...
coseq(t_rev_bacteria_counts,
      ...
      model="kmeans",
      nstart=1, 
      iter.max=50) -> coseq_out_2
```


```{r}
# cluster results
clusters(coseq_out_2) 

# conditional probabilities of cluster membership
assay(coseq_out_2) %>% as.data.frame()
```
