...
Goal:  Find groups (i.e., essentially networks) of bacteria that tend to be found together. Use these to see if bacteria features can be paired down. 
- Reduce the dimensionality of the bacteria strains
...
```{r}
# Using the scaled data gave the following error
# Error in svd(x, nu = 0, nv = k) : infinite or missing values in 'x'
#stats::prcomp(bac_sc) -> prcomp_results

# Using the regular df unscaled
stats::prcomp(bacteria[,-1]) -> prcomp_results
```

```{r}
summary(prcomp_results)

typeof(prcomp_results) # list object therefore I can use map

map(prcomp_results, ~as.data.frame(.))
```
